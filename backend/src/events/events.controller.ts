...
export class EventsController {
  ...
  @ApiOperation({
    summary: 'イベント一括登録',
    description: '監査ログイベントを一括登録する',
  })
  @ApiResponse({ status: 201, description: '登録成功' })
  createBulk(
    @Request() req: any,
    @Body() bulkCreateEventDto: BulkCreateEventDto,
  ) {
    return this.eventsService.createBulk(
      req.user.tenantId,
      bulkCreateEventDto.events,
    );
  }
  ...
  @ApiOperation({
    summary: 'イベント検索',
    description: 'UDMフィールドのfilter条件でイベントを検索する',
  })
  ...
  @ApiOperation({
    summary: 'イベント詳細取得',
    description: '指定IDのイベントをsecurityResults含めて取得する',
  })
