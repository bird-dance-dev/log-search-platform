## プロダクト概要
実務でSIEM製品の導入支援に関わる中で、導入よりも製品を作る側への思いが強くなり、ログ検索プラットフォームをテーマに設計・実装しました。\
...
## テスト
認証・認可フローにて、ユニットテストとe2eテストの2層構成で検証しています。

### ユニットテスト
認証・認可のロジックを外部依存（DB・JWT）をモック化して単体検証します。
```bash
cd backend
npm test
```
| ファイル | 検証内容 |
|---------|---------|
| `auth.service.spec.ts` | ログイン成功/失敗、JWTの正当性、認証エラーメッセージの同一性（情報漏洩防止） |
| `functional-roles.guard.spec.ts` | デコレータ未設定時の拒否、ロール別アクセス制御 |

### e2eテスト
実際のDB・HTTPリクエストを通して、認証認可パイプライン全体を検証します。(`auth.e2e-spec.ts`)
```bash
docker compose up -d
cd backend
npm run test:e2e
...
| テストケース | 検証内容 |
|------------|---------|
| 認証 | トークンなしで401、ログインでトークン取得 |
| 機能ロール | 一般ユーザーで管理者限定エンドポイントにアクセスで403、管理者でアクセス成功 |
| テナント分離 | テナントA/Bの検索結果にイベントIDの重複がないこと |
| データロール | 限定権限ユーザーの検索結果が、許可されたnamespaceのみに制限されること |
